...
<template>
  <section>
    <el-form :inline="true" :model="filters" label-position="left" ref="filters" label-width="120px">
      <div class="search_top">
        <el-row>
          <el-col :span="6">
            <el-form-item label="商户名称" prop="mname">
              <el-input v-model.trim="filters.mname" placeholder="请输入商户名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item label="交易金额" prop="orderAmt">
              <el-input v-model="filters.orderAmt" type="number" placeholder="请输入交易金额"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="预授权类型" prop="payAuthStatus">
              <el-select v-model="filters.payAuthStatus" clearable placeholder="请选择">
                <el-option value="1" label="预授权"></el-option>
                <el-option value="2" label="撤销"></el-option>
                <el-option value="3" label="押金消费"></el-option>
                <el-option value="4" label="押金退款"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <div v-loading="listLoading">
      <el-table :data="users" border stripe highlight-current-row>
        <el-table-column prop="mchntOrderNo" label="订单号"/>
        <el-table-column prop="channelOrderNo" label="第三方订单号"/>
        <el-table-column prop="mname" label="商户名称"/>
        <el-table-column prop="payAuthStatus" label="预授权类型" :formatter="formatterPayAuthStatus"/>
        <el-table-column prop="orderAmt" label="交易金额"/>
        <el-table-column prop="preType" label="支付方式" />
        <el-table-column :formatter="formatCreate_time" label="付款时间"/>
        <el-table-column prop="status" label="交易状态"  :formatter="formatterPreType"/>
        <el-table-column label="操作" align="center" width="180">
          <template slot-scope="scope">
            <el-button type="success" size="mini" :disabled="scope.row.status ===  1" @click="fillOrder(scope.$index, scope.row)">同步</el-button>
            <el-button type="info" size="mini" @click="detailsOrder(scope.$index, scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    formatterPreType(row, column){
      return row.status === 0 ? '初始' : row.status === 1 ? '成功' : row.status === 2 ? '失败' : row.status === 5 ? '支付中' : '未知'
    },
    formatterPayAuthStatus(row, column){
      return row.payAuthStatus === '1' ? '预授权' : row.payAuthStatus === '2' ? '撤销' : row.payAuthStatus === '3' ? '押金消费' : row.payAuthStatus === '4' ? '押金退款' : '未知'
    },
    formatCreate_time(row, column) {
      return row.txnEndTs ? util.formatDate.format(
        new Date(row.txnEndTs),
        "yyyy/MM/dd hh:mm:ss"
      ) : '';
    },
    ...
    detailsOrder(index, row) {
      ...
      this.$nextTick(() => {
        yichangdingdanquery({ out_trade_no: row.mchntOrderNo, terminal_id: row.termId, pcType:'PC' }).then(res => {
          this.detailsForm = res.preAuthHistory
        });
      });
    },
    submitOrder() {
    ...
    getList() {
      ...
      para.pageNumber = this.page.toString();
      para.startTime = para.dateTime ? para.dateTime[0] : "";
      para.endTime = para.dateTime ? para.dateTime[1] : "";
      queryError(para).then(res => {
